...
app.post('/', async (req: Request, res: Response) => {
	...
	console.log('ID de session recibida', req.body);
	try {
...
function handleError(error: any, res: Response){
	...
	if (error.code === 'ECONNREFUSED'){
		console.error('ERROR: No es posible conectar con el servidor');
		res.status(503).send('ECONNREFUSED: No es posible conectar con el servidor de medios');
		return;
	}
	if(error.code === 'DEPTH_ZERO_SELF_SIGNED_CERT' || error.code.includes('SELF_SIGNED_CERT')){
		res.status(401).send('ERROR: Certificado auto firmado '+ OPENVIDU_URL);
		return;
	}
	res.status(statusCode).send('ERROR: No es posible crear una sesión');
}
